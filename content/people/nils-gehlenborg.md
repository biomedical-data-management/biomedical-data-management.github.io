---
hidden: true
order: 1
type: person
groups:
    - invited
thumbnail: images/people/nils-gehlenborg.jpg
title: Nils Gehlenborg
details: Associate Professor / Department of Biomedical Informatics/ Harvard University
summary: |
  His research group develops visual interfaces and computational techniques that enable scientists and clinicians to efficiently
  interact with biomedical data. They developed the [HubMAP data portal](https://portal.hubmapconsortium.org/)
  for discovering, visualizing and downloading standardized multi-modal spatial and single-cell data from healthy human tissues.
links:
    - href: https://hidivelab.org/team/members/nils-gehlenborg/
      icon: fa-home
      label: Homepage
    - href: https://x.com/ngehlenborg
      icon: fa-brands fa-x-twitter
    - href: https://scholar.google.com/citations?user=YEcBVFAAAAAJ
      icon: fa-graduation-cap
    - href: https://www.linkedin.com/in/ngehlenborg/
      icon: fa-brands fa-linkedin
---